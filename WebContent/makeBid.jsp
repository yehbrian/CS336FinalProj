...
<body>
	<%
		if ((session.getAttribute("user") != null)) {
			...
			String bids2 = String.format("select * from Bids where itemID = '%d' AND amount= (SELECT max(amount) FROM Bids where itemID = '%d')", itemID, itemID);
			//System.out.println(bids2);

			ResultSet result2 = stmt2.executeQuery(bids2);
			Statement stmt3 = con.createStatement();
			String bids3 = String.format("select * from Bids where itemID = '%d' AND amount= (SELECT min(amount) FROM Bids where itemID = '%d')", itemID, itemID);
							...
							if (Integer.parseInt(auto) == 0) {
								...
								if (result2.getInt("automaticBidding") == 1) {
									if (((result3.getFloat("amount") * -1)) >= amount
										...
										ps.setInt(2, Integer.parseInt(session.getAttribute("userID").toString()));
										ps.setInt(3, itemID);
										ps.setFloat(4, (result3.getFloat("amount")*-1 + result.getFloat("increment")));
										ps.setInt(5, 1);
										long h = System.currentTimeMillis();
										java.sql.Timestamp time = new java.sql.Timestamp(h);
										ps.setTimestamp(6, time);
